                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-10 m-auto">
                                <form action="{{ route('b.admin.update', $admin->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group align-items-center">
                                            <input type="radio" class="btn-check" name="status" value="1" {{ $admin->status == 1 ? 'checked' : '' }}
                                                id="status-yes" autocomplete="off" checked >
                                            <label class="btn btn-outline-success w-50 m-0" for="status-yes">{{ __('Active') }}</label>

                                            <input type="radio" class="btn-check" name="status" value="0" {{ $admin->status == 0 ? 'checked' : '' }}
